...
function options(values,current){return Object.entries(values).map(([k,v])=>`<option value="${k}" ${current===k?"selected":""}>${v}</option>`).join("")}
...
function addRevisionChange(change={}){
 ...
 row.querySelector("button").onclick=()=>{row.remove();syncRevisionRecord(false);dirtyFields.add("revisionRecord");preserveLocal()};$("revisionChanges").append(row);
}
function locateExact(source,quote,strict){
 const start=source.indexOf(quote);
 if(!quote||start<0||source.indexOf(quote,start+1)>=0){if(strict)throw Error("修改前后原文必须能在对应全文中唯一定位，请扩大引用片段。");return {start:0,end:0,text:quote}}
