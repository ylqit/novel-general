...
41. `scene_prose_editor` reviews every chapter. Planning, reader-experience, character, anti-AI, and canon roles are additive and never replace scene review.
42. Finalization requires a current `human_story_review_v3` accept bound to candidate, chapter-contract, reader-promise-ledger, causal-simulation and immutable review-bundle hashes. All ten checks must pass, with exact key-turn, character-choice/emotion and reader-gain spans. Repair requires a structured annotation; redirect names chapter direction or outline revision.
...
49. Human prose edits are complete replacement candidates for a validated repair plan, submitted as `agent=human`; they consume the normal repair budget and rerun the complete gate and independent-review barrier.
