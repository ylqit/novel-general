...
def validate_character_expression_review(
    root: Path,
    payload: Any,
    *,
    manifest_inputs: Iterable[str],
) -> list[str]:
    """Validate a range review and every cited source span without canonical writes."""

    errors: list[str] = []
    if not isinstance(payload, dict):
        return ["character expression review must be a JSON object."]
    required = {
        "schema",
        "scope",
        "source_hashes",
    ...
    if extra:
        errors.append("unknown fields: " + ", ".join(sorted(extra)))
    if payload.get("schema") != CHARACTER_REVIEW_SCHEMA:
        errors.append(f"schema must be {CHARACTER_REVIEW_SCHEMA}.")
    scope = payload.get("scope")
    if not isinstance(scope, dict) or set(scope) != {"from_chapter", "to_chapter"}:
        errors.append("scope must contain from_chapter and to_chapter only.")
        return errors
    start, end = scope.get("from_chapter"), scope.get("to_chapter")
    if not isinstance(start, int) or not isinstance(end, int) or start <= 0 or end < start:
        errors.append("scope must be a valid positive chapter range.")
        return errors
    declared = {str(item) for item in manifest_inputs}
    hashes = payload.get("source_hashes") if isinstance(payload.get("source_hashes"), dict) else {}
    source_texts: dict[str, str] = {}
    for source, digest in hashes.items():
        source = str(source)
        if source not in declared:
            errors.append(f"source_hashes entry is not declared by the manifest: {source}.")
            continue
        path = root / source
        if not path.is_file():
            errors.append(f"review source does not exist: {source}.")
            continue
        if sha256(path.read_bytes()).hexdigest() != digest:
            errors.append(f"source hash changed: {source}.")
            continue
        source_texts[source] = path.read_text(encoding="utf-8").lstrip("\ufeff")

    known = read_json(root / "10_bible" / "characters.json", [])
    known_ids = {
        str(item.get("id")) for item in known
        if isinstance(item, dict) and item.get("id")
    } if isinstance(known, list) else set()
    character_reviews = payload.get("character_reviews")
    if not isinstance(character_reviews, list) or not character_reviews:
        errors.append("character_reviews must contain at least one reviewed character.")
    else:
        seen_ids: set[str] = set()
        for index, review in enumerate(character_reviews):
            prefix = f"character_reviews[{index}]"
            expected = {"character_id", "verdict", "dimensions", "summary", "evidence"}
            if not isinstance(review, dict) or set(review) != expected:
                errors.append(f"{prefix} must contain exactly: {', '.join(sorted(expected))}.")
                continue
            character_id = str(review.get("character_id") or "")
            if character_id not in known_ids or character_id in seen_ids:
                errors.append(f"{prefix}.character_id must be unique and declared in characters.json.")
            seen_ids.add(character_id)
            if review.get("verdict") not in {"pass", "risk", "blocked"}:
                errors.append(f"{prefix}.verdict must be pass, risk, or blocked.")
            dimensions = review.get("dimensions")
            if not isinstance(dimensions, dict) or set(dimensions) != set(CHARACTER_REVIEW_DIMENSIONS):
                errors.append(f"{prefix}.dimensions must cover every character-expression review dimension.")
            elif any(value not in {"pass", "risk", "not_observed"} for value in dimensions.values()):
                errors.append(f"{prefix}.dimensions values must be pass, risk, or not_observed.")
            validate_review_evidence(review.get("evidence"), source_texts, errors, prefix=prefix)

    chapter_reviews = payload.get("chapter_reviews")
    expected_chapters = set(range(start, end + 1))
    observed_chapters: set[int] = set()
    if not isinstance(chapter_reviews, list):
        errors.append("chapter_reviews must be a list.")
    else:
        for index, review in enumerate(chapter_reviews):
            prefix = f"chapter_reviews[{index}]"
            expected = {"chapter_number", "verdict", "risks", "summary", "evidence"}
            if not isinstance(review, dict) or set(review) != expected:
                errors.append(f"{prefix} must contain exactly: {', '.join(sorted(expected))}.")
                continue
            chapter_number = review.get("chapter_number")
            if chapter_number not in expected_chapters or chapter_number in observed_chapters:
                errors.append(f"{prefix}.chapter_number must be unique inside the declared range.")
            elif isinstance(chapter_number, int):
                observed_chapters.add(chapter_number)
            if review.get("verdict") not in {"pass", "risk", "blocked"}:
                errors.append(f"{prefix}.verdict must be pass, risk, or blocked.")
            if not isinstance(review.get("risks"), list):
                errors.append(f"{prefix}.risks must be a list.")
            validate_review_evidence(review.get("evidence"), source_texts, errors, prefix=prefix)
        missing_chapters = sorted(expected_chapters - observed_chapters)
        if missing_chapters:
            errors.append("chapter_reviews do not cover: " + ", ".join(str(item) for item in missing_chapters) + ".")
    if not isinstance(payload.get("cross_chapter_findings"), list):
        errors.append("cross_chapter_findings must be a list.")
    if payload.get("verdict") not in {"pass", "risk", "blocked"}:
        errors.append("verdict must be pass, risk, or blocked.")
    return errors
...
def validate_review_evidence(
    ...
    *,
    ...
    if not isinstance(evidence, list) or not evidence:
        errors.append(f"{prefix}.evidence must contain at least one exact source span, including pass verdicts.")
        return
    for index, item in enumerate(evidence):
        item_prefix = f"{prefix}.evidence[{index}]"
        expected = {"source_path", "start", "end", "excerpt"}
        if not isinstance(item, dict) or set(item) != expected:
            errors.append(f"{item_prefix} must contain source_path, start, end, and excerpt only.")
            continue
        source = str(item.get("source_path") or "")
        text = source_texts.get(source)
        start, end = item.get("start"), item.get("end")
        if text is None:
            errors.append(f"{item_prefix}.source_path must have a validated source hash.")
        elif not isinstance(start, int) or not isinstance(end, int) or start < 0 or end <= start or end > len(text):
            errors.append(f"{item_prefix} span is outside the source text.")
        elif text[start:end] != item.get("excerpt"):
            errors.append(f"{item_prefix}.excerpt must exactly match the declared source span.")
