    ...
    def do_GET(self) -> None:  # noqa: N802
        try:
            ...
            if chapter_page_match:
                ...
                if (chapter.get("review") or {}).get("available"):
                    prefix = (
...
def _is_workspace_page_route(path: str) -> bool:
    return bool(
        ...
        or re.fullmatch(
            r"/projects/project_[0-9a-f]{20}(?:/(?:design|sources|fanfiction|planning|knowledge|publication|recovery))?",
            path,
        )
...
<meta name="viewport" content="width=device-width,initial-scale=1"><title>小说创作工作台</title>
...
function dashboardHtml(state){const a=state.next_action||{};return `<section class="card"><div class="eyebrow">Current safe action</div><div class="action-box"><span class="badge ${badgeClass(a.status)}">${esc(statusName(a.status))}</span><h3>${esc(a.task_type||"当前生产状态")}</h3><p>${esc(actionSummary(a))}</p>${a.chapter_number?`<p><a href="/projects/${esc(state.project.id)}/chapters/${a.chapter_number}">打开第 ${a.chapter_number} 章工作台</a></p>`:""}${a.blocked_by&&a.blocked_by!=="none"?`<p class="small muted">阻断：${esc(a.blocked_by)}</p>`:""}<div class="button-row">${actionControls(state)}</div></div><div id="action-status" class="status-line"></div></section>${candidateHtml(state)}${canonicalCandidateHtml(state)}${semanticCandidateHtml(state)}${chapterConfirmationHtml(state)}<section class="card"><h3>Codex 工单</h3><p class="small muted">浏览器不会提交任意 Prompt；后端只接收当前 Manifest 的 task_id，并把声明输入复制到隔离 staging。</p><div id="job-list">${jobsHtml(state.agent_jobs)}</div></section>`}
async function renderProject(projectId,section="dashboard",chapter=null){const [state,workspace]=await Promise.all([get(`/api/projects/${projectId}`),get("/api/workspace")]);window.projectState=state;window.workspaceState=workspace;setWorkspace(`${workspace.workspace} · ${state.project.title}`);let content;if(chapter){const ch=await get(`/api/projects/${projectId}/chapters/${chapter}`);content=`<section class="card"><div class="eyebrow">Chapter ${chapter}</div><h2>第 ${chapter} 章创作中心</h2><p>${esc(actionSummary(ch.production))}</p>${ch.chapter_context.final.exists?'<p class="notice">正式章节已经存在；当前页面继续处理语义状态与关闭流程。</p>':'<p class="notice warn">当前尚无可进入审稿台的章节草稿。先完成当前安全动作。</p>'}<a class="button secondary" href="/projects/${projectId}">返回项目驾驶舱</a></section>${dashboardHtml(state)}`}else if(section==="dashboard"){content=dashboardHtml(state)}else{const info=sectionText[section]||["项目工作区","该工作区由当前生产状态驱动。"],mode=state.project.creation_mode;content=`<section class="card"><div class="eyebrow">${esc(section)}</div><h2>${esc(info[0])}</h2><p class="lead">${esc(info[1])}</p><div class="action-box"><span class="badge ${badgeClass(state.next_action.status)}">${esc(statusName(state.next_action.status))}</span><p>${esc(actionSummary(state.next_action))}</p><div class="button-row">${actionControls(state)}</div></div>${section==="sources"&&mode==="original"?'<p class="notice">原创项目不需要原著资料流程。</p>':""}${section==="fanfiction"&&mode==="original"?'<p class="notice">这是原创项目，不启用同人 Canon 路线。</p>':""}<p class="small muted">本页遵守同一个项目状态机；打开页面本身不会触发 apply、finalize、semantic apply 或 chapter close。</p></section>`}app.className="";app.innerHTML=`<section class="panel" style="margin-bottom:22px"><span class="badge">${esc(modeName(state.project.creation_mode))}</span><h2>${esc(state.project.title)}</h2><p class="muted">${esc(state.project.relative_config)}</p></section><div class="project-layout">${projectNav(state,section)}<div class="grid" style="grid-template-columns:1fr">${content}</div></div>`;bindProjectActions(projectId);if(state.agent_jobs.some(j=>["starting","running","cancelling"].includes(j.status)))setTimeout(()=>renderProject(projectId,section,chapter).catch(showError),1800)}
...
async function boot(){if(currentPath==="/projects/new")return renderNewProject();const match=currentPath.match(/^\/projects\/(project_[0-9a-f]{20})(?:\/(design|sources|fanfiction|planning|knowledge|publication|recovery))?$/);if(match)return renderProject(match[1],match[2]||"dashboard");const chapter=currentPath.match(/^\/projects\/(project_[0-9a-f]{20})\/chapters\/([1-9][0-9]*)$/);if(chapter)return renderProject(chapter[1],"dashboard",Number(chapter[2]));return renderHome()}
