...
<section data-panel="跨界宪法"><h2>跨界宪法</h2><p>按实际 allowed_elements 动态处理身体、灵魂、感知、能量、代价、反制、身份、死亡、返回和不可逆后果，不建立全作品两两矩阵。</p><pre>longform-engine fanfiction design-task project.yaml</pre></section>
<section data-panel="主世界适配器"><h2>主世界适配器</h2><p>每部来源先独立适配宿主世界，再进入跨界宪法；只为当前卷实际交互建立例外。</p><pre>每部来源 → 宿主世界适配器 → 跨界宪法 → 当前卷例外规则</pre></section>
