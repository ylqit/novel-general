...
37. `chapter_contract_v4` and rolling plans use `chapter_turn`, `reveal_boundary`, `emotional_aftereffect`, `ending_mode`, `ending_intent`, suspense/resolution boundaries, `reader_gain`, reader-promise actions and an approved causal-simulation reference; `hook`, `hook_mode`, `plot_obligation`, `irreversible_action`, card-level `dramatic_freedom`, `information_release` and every earlier alias must fail validation instead of being silently normalized.
...
40. The author reads only `chapter_story_brief_v4`; it includes bounded necessary character voice and relevant facts. Fact IDs, source hashes, promise IDs, pattern codes, planning ledgers, causal-simulation hashes, raw RAG packets, Graph/SQLite vocabulary, and evidence provenance remain in internal planning/editor/archivist lanes.
...
42. Finalization requires a current `human_author_revision_v3` final lock plus `human_story_review_v6` accept bound to candidate, chapter contract, human intent, Story Brief basis, reader-promise ledger, causal simulation, immutable review bundle and human revision. Humans supply exact key-turn, character-choice/emotion and reader-gain evidence; every finding needs an explicit disposition.
43. `reader_promise_ledger_v1` describes author-side expectation management, not clicks, retention, comments or any other observed reader behavior. Passing the target chapter warns; passing the latest chapter blocks direction until explicit deferral or outline revision.
44. `arc_causal_simulation_v1` is a human-approved rolling planning constraint, not a prose template or world fact. Changed basis hashes make it stale, and downstream cards/tasks are invalidated transactionally.
...
51. `chapter_story_brief_basis_v2` binds every projection that can change the author work order, including current `human_chapter_intent_v1`. A writing task may be reused only when task status, contract, intent, basis, Markdown bytes and the active manifest are all current.
52. `human_chapter_intent_v1` starts blank and must be completed by a human after direction selection. No frontend, Agent or CLI may prefill its creative answers; stale selection or contract hashes block writing.
53. `chapter_coedit_session_v1` stores bounded decisions, spans, option IDs, intent and candidate hashes rather than full prompts or unrelated chat. Every applied suggestion creates a complete workbench candidate and cannot bypass repair budget.
