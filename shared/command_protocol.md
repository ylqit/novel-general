...
| `/工程工单` | `longform-engine agent-task brief project.yaml TASK_OR_PATH` | `project.yaml`、`TASK_OR_PATH` | 只读 | 将 `AgentTaskManifest v1` 渲染成 Codex / ClaudeCode 可执行工作单。 |
...
| `/工程语义验稿` | `longform-engine gate-check project.yaml --chapter N --semantic` | `--chapter N` | `50_workbench/gate_artifacts/` | 执行含语义检查的章节门禁。 |
...
- `editorial review` writes role task files for `planning_chief_editor`, `writing_agent`, `anti_ai_editor`, `serial_verifier`, and `executive_editor`.
- Review JSON records `severity_counts`, `review_round`, `unresolved_items`, `conditional_pass_streak`, and `need_human_reasons`.
