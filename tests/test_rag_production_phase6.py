...
def test_production_rag_runner_writes_claim_grade_engine_evidence(tmp_path, monkeypatch):
        ...
        json.dumps(
            {
                ...
                "dataset_id": "phase6-production-fixture",
                "queries": queries,
            },
            ensure_ascii=False,
        ),
    ...
    monkeypatch.setattr(
        ...
        lambda _config, with_embeddings: SimpleNamespace(chapters=500, chunks=500, embeddings=500),
    )
    monkeypatch.setattr(
        "longform_engine.rag.production_benchmark.build_embeddings",
        lambda _config: 0,
    )
