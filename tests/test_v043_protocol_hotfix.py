...
def test_semantic_rag_materialization_and_vector_failure_rollback(tmp_path, monkeypatch):
    ...
    original_sync = rag_pipeline.sync_vector_store

    def sync_then_fail(config, records):
        original_sync(config, records)
        raise RuntimeError("simulated vector commit failure")

    monkeypatch.setattr(rag_pipeline, "sync_vector_store", sync_then_fail)
