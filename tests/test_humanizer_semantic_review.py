...
def test_prose_naturalness_semantic_task_is_strict_and_surfaces_in_production_next(tmp_path):
    config, root, candidate = seed_prose_naturalness_project(tmp_path, milestones=[1])
    ...
    assert "prose_revision_semantic_review:ch001:prose_naturalness:v1" in check.next_command
    manifest = load_manifest(root, "prose_revision_semantic_review:ch001:prose_naturalness:v1")
    validation = validate_manifest_strict(root, manifest)
    assert validation.ok, validation.errors
    assert manifest["io"]["output"] == {
        "path": "50_workbench/prose_naturalness_tasks/ch001.semantic_review.json",
    ...
    assert action["task_type"] == "prose_revision_semantic_review"
    assert action["task_id"] == "prose_revision_semantic_review:ch001:prose_naturalness:v1"


def test_agent_first_normalizer_accepts_prose_naturalness_evidence_review(tmp_path):
    config, root, candidate = seed_prose_naturalness_project(tmp_path, milestones=[1])
    prose_naturalness_check(config, chapter_number=1, file_path=candidate)
...
def test_prose_naturalness_semantic_pass_allows_submit_and_applies_both_tasks(tmp_path):
    config, root, candidate = seed_prose_naturalness_project(tmp_path, milestones=[1])
    prose_naturalness_check(config, chapter_number=1, file_path=candidate)
    ...
    output = write_semantic_result(root)
    review = validate_prose_naturalness_output(config, root, output)
    assert review.ok
    ...
    statuses = {
        ...
        if item["task_type"] in {"prose_naturalness", "prose_revision_semantic_review"}
    }
    assert statuses == {"prose_naturalness": "submitted", "prose_revision_semantic_review": "applied"}


def test_production_loop_validates_existing_prose_naturalness_semantic_output_without_apply(tmp_path):
    config, root, candidate = seed_prose_naturalness_project(tmp_path, milestones=[1])
    prose_naturalness_check(config, chapter_number=1, file_path=candidate)
    ...
    assert result["steps"][0]["action"] == "prose_revision_semantic_validate"
    task = next(
        item
        for item in list_manifests(root, chapter_number=1)
        if item["task_type"] == "prose_revision_semantic_review"
    )
    assert task["status"] == "validated"
    assert not (root / "40_manuscript" / "final" / "ch001.md").exists()


def test_prose_naturalness_semantic_rejects_output_path_outside_declared_slot(tmp_path):
    config, root, candidate = seed_prose_naturalness_project(tmp_path, milestones=[1])
    prose_naturalness_check(config, chapter_number=1, file_path=candidate)
    outside = root / "50_workbench" / "prose_naturalness_tasks" / "wrong.json"
    write_json(outside, {"schema": EVIDENCE_REVIEW_SCHEMA})
    before = canonical_snapshot(root)

    with pytest.raises(ValueError, match="Prose-naturalness semantic result must be"):
        prose_naturalness_semantic_validate(config, chapter_number=1, file_path=outside)
    assert canonical_snapshot(root) == before


def test_prose_naturalness_semantic_invalid_evidence_does_not_pollute_canonical_state(tmp_path):
    config, root, candidate = seed_prose_naturalness_project(tmp_path, milestones=[1])
    prose_naturalness_check(config, chapter_number=1, file_path=candidate)
    output = write_semantic_result(root)
    payload = read_json(output)
    payload["verdict"] = "repair"
    payload["findings"] = [prose_naturalness_blocking_finding(root, evidence_ids=["ch001.prose_naturalness_candidate.md@0:99999"])]
...
def test_prose_naturalness_semantic_pass_verdict_cannot_override_fact_change(tmp_path):
    config, root, candidate = seed_prose_naturalness_project(tmp_path, milestones=[1])
    prose_naturalness_check(config, chapter_number=1, file_path=candidate)
    output = write_semantic_result(root)
    assert validate_prose_naturalness_output(config, root, output).passed
    payload = read_json(output)
    payload["findings"] = [prose_naturalness_blocking_finding(root)]
    write_json(output, payload)
    before = canonical_snapshot(root)
    ...
    payload["verdict"] = "repair"
    write_json(output, payload)
    repair = validate_prose_naturalness_output(config, root, output)
    assert repair.ok
    assert not repair.passed
    assert "PROSE_REVISION_FACT_DRIFT" in repair.blocking_findings
    assert "creative prose-naturalness-task" in repair.next_command
    assert canonical_snapshot(root) == before


def test_prose_naturalness_candidate_change_after_review_requires_a_new_review(tmp_path):
    config, root, candidate = seed_prose_naturalness_project(tmp_path, milestones=[1])
    prose_naturalness_check(config, chapter_number=1, file_path=candidate)
    output = write_semantic_result(root)
    assert validate_prose_naturalness_output(config, root, output).passed
    ...
    refreshed = prose_naturalness_check(config, chapter_number=1, file_path=candidate)
...
def test_low_change_prose_naturalness_candidate_still_requires_unified_dual_prose_review(tmp_path):
    config, root, candidate = seed_prose_naturalness_project(tmp_path, milestones=[])
    ...
    assert check.passed
    assert check.semantic_review_required
    assert "dual_prose_semantic_review" in check.semantic_review_reasons
    assert (root / "50_workbench" / "prose_naturalness_tasks" / "ch001.semantic_review.agent_task.json").exists()


def test_strict_mode_requires_review_even_without_milestone_or_change_risk(tmp_path):
    config, _root, candidate = seed_prose_naturalness_project(tmp_path, milestones=[])
...
def seed_prose_naturalness_project(tmp_path, *, milestones):
...
def write_semantic_result(root):
    source = root / "50_workbench" / "repair_candidates" / "ch001.prose_naturalness_candidate.md"
    payload = {
    ...
    output = root / "50_workbench" / "prose_naturalness_tasks" / "ch001.semantic_review.json"
    write_json(output, payload)
    return output


def prose_naturalness_blocking_finding(root, *, evidence_ids=None):
    ids = evidence_ids or [
        "40_manuscript/draft/ch001.md@0:8",
        "50_workbench/repair_candidates/ch001.prose_naturalness_candidate.md@0:8",
    ]
    return {
        "code": "PROSE_REVISION_FACT_DRIFT",
        "severity": "P1",
        "certainty": "confirmed",
...
def validate_prose_naturalness_output(config, root, output):
    manifest = load_manifest(root, "prose_revision_semantic_review:ch001:prose_naturalness:v1")
    control = validate_production_agent_result(root, manifest, result_file=output)
    assert control.ok, control.normalization.errors
    return prose_naturalness_semantic_validate(config, chapter_number=1, file_path=output)
