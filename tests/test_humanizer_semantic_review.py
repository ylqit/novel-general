...
def test_humanizer_semantic_task_is_strict_and_surfaces_in_production_next(tmp_path):
    ...
    assert "prose_revision_semantic_review:ch001:humanizer:v4" in check.next_command
    manifest = load_manifest(root, "prose_revision_semantic_review:ch001:humanizer:v4")
    ...
    assert action["task_id"] == "prose_revision_semantic_review:ch001:humanizer:v4"
...
def test_humanizer_semantic_pass_allows_submit_and_applies_both_tasks(tmp_path):
    ...
    statuses = {
        ...
        if item["task_type"] in {"humanize", "prose_revision_semantic_review"}
    }
    assert statuses == {"humanize": "submitted", "prose_revision_semantic_review": "applied"}
...
def test_low_change_humanizer_candidate_still_requires_unified_dual_prose_review(tmp_path):
    ...
    assert (root / "50_workbench" / "humanizer_tasks" / "ch001.semantic_review.agent_task.json").exists()
