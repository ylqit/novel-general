...
def seed_formal_pair(tmp_path: Path):
    ...
    for run_id, host, workflow in (
        ("codex-current-10", "codex", "current-workflow"),
        ("codex-variant-10", "codex", "variant-workflow"),
    ):
        init_benchmark(
            config,
            run_id=run_id,
        ...
        for chapter in range(1, 11):
            version_label = "甲稿" if run_id == "codex-current-10" else "乙稿"
...
def test_formal_blind_review_aggregates_three_independent_judges(tmp_path):
    ...
    pack = create_blind_review_pack(
        ...
        run_ids=["codex-current-10", "codex-variant-10"],
        seed="fixed-private-seed",
    )
    public_manifest = (
        root / pack.public_dir / "manifest.json"
    ).read_text(encoding="utf-8")
    assert "codex-current-10" not in public_manifest
    assert "codex-variant-10" not in public_manifest
    ...
    write_production_rag_evidence(root, "codex-current-10")
    write_production_rag_evidence(root, "codex-variant-10")
    ...
    assert comparison.quality_evidence_complete
    assert not comparison.evidence_gaps
    records = json.loads(
        (
            root
            / "70_runtime"
    ...
    (tmp_path / "codex-current-10" / "ch001.md").write_text(
...
def test_blind_pack_detects_source_change_and_submission_identity_leak(tmp_path):
    config, root = seed_formal_pair(tmp_path)
    changed = tmp_path / "codex-current-10" / "ch010.md"
    ...
    attach_benchmark_source(
        config,
        run_id="codex-current-10",
        source_dir=tmp_path / "codex-current-10",
    )
    pack = create_blind_review_pack(
        config,
    ...
    payload["run_id"] = "codex-current-10"
