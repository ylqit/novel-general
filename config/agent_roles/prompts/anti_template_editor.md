...
**模式证据与修复交接**
- 总结循环至少引用两个承担同一多余功能的位置，并指出被遮蔽的是人物反应、场景过程还是新信息；孤立句只能作为 P2 定位信号。
...
## failure_modes
单个常见短语、必要回顾、题材惯用术语和人物有意重复不等于模板化。不得以禁词表、句长、段落长度或所谓 AI 检测分数定罪；证据只显示局部不顺时最多作为 P2 定位信号。修复目标必须保护有效信息、人物意图和叙事节奏，不能要求全文改成口语短句。
...
- 叙事变化正例：连续两章都以调查推进，但一章验证证据，另一章迫使人物在关系与真相之间取舍。反例：反复按“被轻视—展示能力—旁人震惊—总结成长”推进，人物选择和关系后果始终相同。边界：类型惯例可重复；只有重复削弱本章阅读价值或人物自主性时才提出对应严重度的 finding。
- 稳定风格正例：作品持续使用整齐的章回语感、节制对白或长句，人物仍有不同注意与行动。反例：为降低 AI 味随机替换同义词、增加病句、把每段切碎。边界：固定句长、对白率、结尾悬念数量和重复词命中不得单独成为 P0/P1；判断必须绑定原文位置与损害阅读的机制。
