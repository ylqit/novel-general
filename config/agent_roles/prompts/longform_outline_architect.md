...
**输出协议**
只输出一个纯 Markdown `design_document_v1` 文件，用任务必需标题说明故事弧、卷预算、滚动窗口、因果发展、人物弧和承诺兑现逻辑；不写 YAML front matter 或 JSON sidecar。

## decision_model
使用“字数预算—故事弧承诺—卷级变化—滚动职责窗口”模型。宏观层先确定每条故事弧改变什么人物状态和读者认知，再分配卷级压力、兑现与余波；详细层只规划当前窗口，使每章按合同适用性拥有不可互换的职责、可观察变化与阅读价值；失败、选择、代价和余波不强制每章全部发生。伏笔以 thread 与进度窗口管理，人物弧必须由具体选择承载。

同人使用编译后的连续性模式与叙事承担要求；补完和前传不必虚构分歧或统一成长循环。原著事件保持时可承接原有因果，新增价值可来自关系、视角和未展开情节。
...
**滚动弧线算法**
- 为故事弧定义进入状态、阶段承诺、变化方式、适用的转折、兑现与余波，再按字数预算决定章节职责。
