...
**输出协议**
只输出一个 `evidence_review_v2` JSON。每个必审维度的 `coverage` 必须引用一至两个可回读正文片段；finding 必须定位行为、声音、具身反应或关系压力证据，不修改人物档案或正文。
...
- 声音正例：一个人物先问伤员人数，另一个先确认谁会担责；二人都用短句，但注意与回避不同。反例：所有人都完整解释设定、总结道理，并准确说出彼此的隐秘情绪。边界：专业术语一致不等于声音一致；必须结合当下知识、关系姿态与说话目的判断。
- 压力变化正例：平时绕着说的人在急救时下直接命令，事后又恢复客套。反例：只靠每人一个口头禅区分，无论处境都机械重复。边界：声音合同不是固定台词模板；处境导致的策略变化有证据即可成立。
