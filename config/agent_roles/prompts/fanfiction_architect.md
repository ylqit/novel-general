...
- `fixed_host` 的 `default_host_source_id` 必须是 configured source；`fusion_world` 必须为 `null` 并建立“世界规则优先级” claim；`sequential_worlds` 必须为 `null`，且每个“卷宿主世界” claim 都以非空字符串 `volume_ids` 与 configured `host_source_id` 声明卷宿主。
