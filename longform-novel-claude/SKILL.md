...
开书阶段按 `book_ideation -> book_design -> planning task` 推进。Book Design 建立 `story_engine_contract_v1`；纲要由 `planning_bundle_v1` 生成活动卷、滚动窗口、显式 `reader_promise_ledger_v2`、语义义务和 Plot Node 表。`production next` 只有在活动卷有效、未来至少三章为 firm、规划 basis 未漂移、独立语义审查通过且全部重大状态变化节点获得人工决定后才进入写作；普通对话、动作和过渡不创建审批配额。人类从空白表单填写 `human_chapter_intent_v3`；不得代填。作者只读取 `chapter_story_brief_v5`；`chapter_story_brief_basis_v4` 绑定唯一 `chapter_contract_v5`、批准节点、语义义务、滚动窗口、必要事实与 renderer，内部任务为 `chapter_writing_task_v8`。内部 ID、hash、原始 RAG、平台诊断和编辑代码不得进入作者工作单。

同人项目先完成用户级原著证据注册、项目中文资料包、固定 item/bundle/normalization/semantic hash 绑定和每部原著的动态 `identity/design_core` 覆盖，再执行 `fanfiction canon-task` 生成项目独立的原著基线 `semantic_document_v1`；当前卷与当前章再按 `volume_scope/chapter_dependency` 增量补证，`whole_to_cutoff` 只在人工选择后成为硬门禁。资料上传不等于理解：Markdown/TXT/JSON/YAML/PDF/DOCX/JPG/PNG/漫画页/ASS/SSA/SRT/MP3/WAV/MP4/MKV 必须形成带页码、区域、cue 或时间码的 `evidence_reference_v1`。Claude 作为 Host Agent 只读取受控证据包；宿主不能直接理解媒体时，使用解析、OCR、ASR、字幕和关键帧证据包。直接观察、解释推断和创作建议必须分开，独立复核与人工批准前不能进入项目 Canon。Agent 只读 manifest 声明输入并只写一个 `semantic_document_v1` 候选，不读取未声明原作、其他资料项、其他小说或普通研究箱；不得搬运或跨片段重构连续原著、字幕、剧本、漫画文字。全局资料修正只产生升级提案，人工决定后才可 stale 未来依赖或进入 `revision_branch_v2`。非同人项目提及作品名只生成待审研究申请，人工批准前不联网；实际使用原著人物、世界或事件必须切换同人模式。

开放语义任务由 `agent_task_manifest_v5` 调度，包括搜索规划、候选筛选、事实提取、视觉观察、时间线对齐、版本冲突、覆盖分析、分时期人物理解、同人路线、故事架构、章节语义规划、初稿语义审查、分目标文风审查和读者反馈分析。人物、事件、关系、能力、外观和跨界规则不使用封闭表格；中文正文承载完整语义，只有会进入 Canon、图谱或依赖传播的断言才建立带证据 claim。模型记忆只能产生搜索线索。
...
资料操作优先使用 `/打开创作控制台`（`longform-engine studio serve project.yaml`）。批量导入依次执行 `source-library ingest-plan`、`ingest-preview`、`ingest-confirm --approved-by human`、`ingest-apply --approved-by human`；格式处理执行 `process-plan/process-run`；资料级语义工单执行 `source-library task-create/task-validate/task-apply --approved-by human`。Claude 作为 Host Agent 只审阅本地规范化结果和声明证据包；首批没有 Anthropic 直连 API。OpenAI 直连只允许逐任务资料预处理，默认关闭，不得自动回退，也不得用于正文、规划、审稿、Canon 决策或发布判断。

覆盖通过并应用 Canon 后，先执行 `fanfiction story-engine-task/validate/apply`，再执行 `fanfiction design-task/validate`；故事发动机必须选择 OC/SI、原著角色中心或混合路线，并声明主角与原著关系、读者识别承诺和独立原创主线。路线必须交给隔离会话完成 `design-review-task/validate`，最后以 `design-apply --review REVIEW --approved-by human` 应用。跨界路线使用 fixed host、fusion world 或 sequential worlds 拓扑，按实际载荷生成来源适配器，不预建世界两两矩阵。`production next` 会自动路由这些任务。当前章通过 `fanfiction context-status --chapter N` 查看显式必需 claim、带理由递归依赖、结构化来源分区、人物知识、原著事件命运、命名冲突、预算和动态跨界规则；内部只接受 `fanfiction_context_bundle_v3`，作者只看到自然中文，多来源/冲突时保留来源标签，审阅器只读精确 review projection 和 evidence 闭包。`rights status` 是人工声明的来源治理信息，不代表法律核验，也不能授权保存或复现受保护全文。普通创作不受发布权利决定阻断；只有 `publication export --target ...` 要求当前人工 `proceed`，其配置、Canon、逐来源权利声明和政策快照任一变化后必须重新决定。

AI 初稿可通过 `chapter_coedit_session_v2` 反复共编：顾问给 2–3 个方案及影响，人类记录选择后才生成新的完整 workbench 候选；不得直接写 canonical，也不得绕过 P0/P1 repair。每章由 `scene_prose_editor` 和 `anti_template_editor` 独立审稿，阻断项进入不可变 `human_review_bundle_v2`；`reader_payoff_review` 用当前 span 证明实际收益。修订语义复核使用 `prose_revision_semantic_review`，人物表达上下文使用 `character_expression_packet_v2`；连续 `source prose` 不得进入正文、Canon、RAG 或任务包。无阻断后，人类完成最终全文修改并锁定，以 `human_author_revision_v4` 绑定意图、共编来源、前后 span、读者影响和保护项；独立双稿复核后以 `agent=human` 提交并全量复审。`human_story_review_v7` 绑定当前合同、节点、义务及审稿证据。人工锁定后 AI 仅可只读咨询；任何 AI 正文变换都会让终稿、咨询和接受 stale。
