...
无 P0/P1 时冻结 `human_review_bundle_v2`。每章必须完成人类完整修订、独立双稿语义复核、`agent=human` 提交和全量复审，之后才能创建 `human_story_review_v7`。accept 需要关键转折、人物选择/情绪和读者收益三类精确 span，并处置所有 finding；repair 需要结构化批注；redirect 必须返回当前协议重新规划。候选、v5 合同、Story Brief basis、承诺账本、批准节点、语义义务、review bundle 或人工修订任一 hash 漂移都使决定失效。
