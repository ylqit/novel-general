...
> 状态核对（2026-08-29）：远程 Git 已确认存在 annotated `v0.12.0` tag，解引用后与远程 `master` 的 `839e1f0` 发布提交一致。当前环境的 GitHub CLI 未登录，因此本次不能重新核验远程 Actions 和 GitHub Release 制品；下方明确分开记录。原 v0.12–v0.14 分阶段实验仍属未发布历史方案。本文既有 `471 passed` 是该发布提交的历史证据，不是当前同人架构优化工作树的验证结果；当前改动必须在全部功能完成后重新执行一次完整单进程链路验证。
...
- [x] `fanfiction_context_bundle_v2` 使用显式 claim、带理由递归依赖、结构化来源/人物/事件/卷/篇章分区、作者/审阅双投影、命名冲突和可解释预算；旧 v1 不双读，姓名匹配、固定条数及无命中前几条回退已移除并有验收测试。
...
- [ ] 主分支 CI 完整通过。
- [x] 远程存在 annotated `v0.12.0` tag，且解引用到远程 `master` 的 `839e1f0` 发布提交（Git 远程只读核对）。
