...
当前公开稳定版是 `v0.10.0`。安装命令固定到不可变 tag；升级 engine 后必须显式同步 Skill。常规升级应重新运行 doctor，本次本机更新按用户授权跳过安装后验证。
...
py -3 -m pipx install --force 'longform-novel-engine[semantic] @ git+https://github.com/ylqit/novel-general.git@v0.10.0'
...
python3 -m pipx install --force 'longform-novel-engine[semantic] @ git+https://github.com/ylqit/novel-general.git@v0.10.0'
longform-engine skills install --tool all --force
longform-engine doctor --tool all
```
